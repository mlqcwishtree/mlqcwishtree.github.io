...
const eventURL = 'night-chant.json';
fetch(eventURL)
    .then(function (response) {
        return response.json();
    })
...
function updateStats(karmas) {
    ...
    if (karmas.length == 1) {
        ...
        oldLocalStorageItem = JSON.parse(localStorage.getItem('nightChantTimesPulled'))
        newLocalStorageItem = oldLocalStorageItem + 1;
        updateLocalStorage("nightChantTimesPulled", newLocalStorageItem);
    }
    else {
        let newLocalStorageItem = JSON.stringify(karmas);
...
function createBackgroundVideo() {
    if (window.mobileCheck() == true) {
        document.getElementById("app-container").style.backgroundImage = "url('assets/night-chant-bg.jpg')";
    }
    else {
        ...
        backgroundVideo.setAttribute("src", "https://youtube.com/embed/Z9uqW91FKWM?autoplay=1&mute=1&controls=0&playlist=Z9uqW91FKWM&loop=1&loop=1");
...
function wishAnimation(timesPulled, karmas) {
    ...
    if (timesPulled == 1) {
        sourceElement.setAttribute("src", "../assets/videos/1-pull-night-chant.mp4");
    }
    else {
        sourceElement.setAttribute("src", "../assets/videos/10-pull-night-chant.mp4");
    }
...
function placeGrids(appWidth) {
    ...
    gwcContainer.style.fontSize = fontSize = appWidth / 50 + "px";
    ...
    // freeTxt.style.height = appWidth / 10 + "px";
