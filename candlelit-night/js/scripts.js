...
let youtubeKeyCode = "w4uBQ7XkUVY";
...
function draw1() {
    ...
    if (kingCheck > 98) {
        ...
        if (karmaRoll > minSPnum) {
            // console.log("EVENT SP: " + karmaRoll);
            let karmaArray = "eventSP";
            // console.log(eventSP);
            // console.log(standardSSR);
            karma = pickKarma(karmaArray);
            // console.log(karma);
        }
...
function displayKarma(karma, displayedKarma, karmas) {
    if (karma) {
        ...
        // Rarity Icon
        let rarityIconURL = "";
        if (title == "Splendid Glimpse" || title == "Poetry and Wine" || title == "Bound by Love" || title == "Taste of Life") {
            rarityIconURL = "../assets/rarity/candlelit-" + rarity + "-icon.png";
        }
        else {
            rarityIconURL = "../assets/rarity/" + rarity + "-icon.png";
        }
...
function SPAnimation(character, karma) {
    let title = karma.title;
    // console.log(title);
    let appContainer = document.getElementById("app-container");
    let videoContainer = document.getElementById("videoContainer");
    ...
    videoElement.appendChild(sourceElement);
    if (title == "Splendid Glimpse" || title == "Poetry and Wine" || title == "Bound by Love" || title == "Taste of Life") {
        sourceElement.setAttribute("src", "../assets/videos/" + character.toLowerCase() + "-candlelit-1" + ".mp4");
    }
    else {
...
function finalSPAnimation(karma) {
    ...
    if (rarity == "SP") {
        ...
        if (!document.getElementById("finalAnimation")) {
            ...
            videoElement.appendChild(sourceElement);
            if (title == "Splendid Glimpse" || title == "Poetry and Wine" || title == "Bound by Love" || title == "Taste of Life") {
                sourceElement.setAttribute("src", "../assets/videos/" + character.toLowerCase() + "-candlelit-2" + ".mp4");
            }
            else {
...
function displayThumbnails(karmas) {
    ...
    for (let i = 0; i < karmas.length; i++) {
        ...
        let rarityURL = "";
        if (title == "Splendid Glimpse" || title == "Poetry and Wine" || title == "Bound by Love" || title == "Taste of Life") {
            rarityURL = "../assets/rarity/candlelit-" + rarity + "-icon.png";
        }
        else {
            rarityURL = "../assets/rarity/" + rarity + "-icon.png";
        }
