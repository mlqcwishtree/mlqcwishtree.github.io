...
// Get Karma List from oracle.json
let eventSSR = [];

const eventURL = 'oracle.json';
fetch(eventURL)
    .then(function (response) {
        return response.json();
    })
...
function updateStats(karmas) {
    ...
    if (karmas.length == 1) {
        ...
        oldLocalStorageItem = JSON.parse(localStorage.getItem('oracleTimesPulled'));
        newLocalStorageItem = oldLocalStorageItem + 1;
        updateLocalStorage("oracleTimesPulled", newLocalStorageItem);
    }
    else {
        let newLocalStorageItem = JSON.stringify(karmas);
...
function createBackgroundVideo() {
    if (window.mobileCheck() == true) {
        document.getElementById("app-container").style.backgroundImage = "url('/rerun-assets/oracle-bg.jpg')";
    }
    else {
        ...
        backgroundVideo.setAttribute("src", "https://youtube.com/embed/p-vOfQkj9RM?autoplay=1&mute=1&controls=0&playlist=p-vOfQkj9RM&loop=1&loop=1");
...
function wishAnimation(timesPulled, karmas) {
    ...
    if (timesPulled == 1) {
        sourceElement.setAttribute("src", "../assets/videos/1-pull-oracle.mp4");
    }
    else {
        sourceElement.setAttribute("src", "../assets/videos/10-pull-oracle.mp4");
    }
